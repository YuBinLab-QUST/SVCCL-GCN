...
import math
from models.SV import CustomVAE_CapsuleIntegration,CustomVAE_CapsuleLoss
...
class SVCCL(nn.Module):
    def __init__(self, bert, opt, vae_latent_dim):
        super(SVCCL, self).__init__()
        self.opt = opt
        self.bert = bert
        ...
        self.bert_linear = nn.Linear(opt.bert_dim, 768)
        self.vae=CustomVAE_CapsuleIntegration(768, vae_latent_dim)
        self.vae_loss = CustomVAE_CapsuleLoss()
    ...
    def position_weight(self, x, aspect_double_idx, text_len, aspect_len):
        ...
        for i in range(batch_size):
            ...
            for j in range(int(aspect_double_idx[i,0])):
                weight[i].append(1-(aspect_double_idx[i,0]-j)/context_len)
            for j in range(int(aspect_double_idx[i,0]), min(int(aspect_double_idx[i,1])+1,self.opt.max_seq_len)):
                weight[i].append(0)
            for j in range(int(aspect_double_idx[i,1])+1, int(text_len[i])):
    ...
    def forward(self, inputs):
        ...
        x_recon,mu,logvar= self.vae(bert_out)
        output=self.fc(x)
        vaeloss=self.vae_loss( x_recon, mu, logvar, bert_out)
        return output,x_recon, mu, logvar, bert_out,vaeloss