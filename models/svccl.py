...
from model.SV import CustomVAE_CapsuleIntegration,CustomVAE_CapsuleLoss
...
class SVGCN(nn.Module):
    def __init__(self, embedding_matrix, opt):
        super(SVGCN, self).__init__()
        self.opt = opt
        ...
        vae_latent_dim=8
        self.casvae=CustomVAE_CapsuleIntegration(600, vae_latent_dim)
        # self.casvae=CustomCapsuleVAE(opt.bert_dim,vae_latent_dim)
        self.vae_capsule_loss = CustomVAE_CapsuleLoss()
    ...
    def forward(self, inputs):
        ...
        x_recon,mu,logvar= self.casvae(text_out)
        vaeloss = self.vae_capsule_loss( x_recon, mu, logvar, text_out) 
        output = self.fc(x)
        return output,vaeloss
